    ...
    <div className="fixed inset-0 z-[200] flex items-end md:items-center justify-center md:p-4 animate-in fade-in duration-300">
      ...
      >
        ...
        <div className="relative w-full md:w-[48%] h-[28vh] md:h-full group bg-[#060b16] shrink-0">
          ...
          {screenshots.length > 1 && (
            <>
              <button
                onClick={prevSlide}
                className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 bg-black/50 backdrop-blur-xl border border-white/10 rounded-full text-white flex items-center justify-center transition-all hover:bg-[#00d68f] hover:text-black"
              >
                <ChevronLeft size={18} />
              </button>
              <button
                onClick={nextSlide}
                className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 bg-black/50 backdrop-blur-xl border border-white/10 rounded-full text-white flex items-center justify-center transition-all hover:bg-[#00d68f] hover:text-black"
              >
        ...
        <div className="flex flex-col flex-1 overflow-y-auto no-scrollbar bg-[#0d1f6e]">
                      ...
                      {edition.cards && edition.cards.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mt-2">
                          {edition.cards.map((card: Card, cIdx: number) => (
                            <div
                              key={cIdx}
                              className={`flex items-center gap-1 px-2 py-1 rounded-lg border text-[9px] font-black ${isSelected ? "bg-[#00d68f]/10 border-[#00d68f]/20 text-[#00d68f]" : "bg-white/[0.03] border-white/5 text-white/20"}`}
                            >
                              <CreditCard size={9} />
                              {card.quantity > 1 && (
          ...
          {/* Итого */}
          <div className="sticky bottom-0 bg-[#0d1f6e] border-t border-white/5 p-4 md:p-10 md:pt-5">
