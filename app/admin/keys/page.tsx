        ...
        <div className="flex items-center gap-4 mb-10">
          ...
          <div>
            <h1 className="text-3xl font-black uppercase text-white tracking-tighter">
              Загрузка <span className="text-[#ff6b00]">ключей</span>
            </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="flex flex-col gap-4">
                  ...
                  <button
                    ...
                    className={`flex-1 py-3 rounded-2xl font-black text-sm transition-all flex items-center justify-center gap-2 ${
                      region === r.code
                        ...
                        : "bg-white/5 border border-[#ff6b00]/40 text-white/40 hover:text-white"
                    }`}
                  >
            ...
            <div className="bg-[#0a1860]/60 border border-[#ff6b00]/40 rounded-[2rem] p-6">
              ...
              <label className="flex flex-col items-center justify-center gap-3 p-6 border-2 border-dashed border-[#ff6b00]/40 rounded-2xl cursor-pointer hover:border-[#ff6b00]/30 transition-all group">
                <Upload
                  size={24}
                  className="text-white/20 group-hover:text-[#ff6b00]/50 transition-all"
                />
              ...
              <textarea
                ...
                rows={5}
                className="w-full px-4 py-3 bg-white/5 border border-[#ff6b00]/40 rounded-2xl text-white/70 font-mono text-xs focus:outline-none focus:border-[#ff6b00]/40 transition-all placeholder-white/10 resize-none"
              />
              {keysText && (
                <p className="text-[#ff6b00] text-[10px] font-black mt-2">
            ...
            <AnimatePresence>
              ...
              {success && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0 }}
                  className="p-4 bg-[#ff6b00]/10 border border-[#ff6b00]/40 rounded-2xl text-[#ff6b00] text-sm font-bold"
                >
                  {success}
                </motion.div>
              )}
            ...
            <button
              ...
              disabled={loading}
              className="w-full py-5 bg-[#ff6b00] text-black font-black uppercase text-sm rounded-2xl hover:shadow-[0_0_30px_rgba(99,243,247,0.3)] transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
          ...
          <div className="flex flex-col gap-4">
                    ...
                    <button
                      ...
                      className={`flex items-center justify-between p-3 rounded-2xl border transition-all ${
                        filterDenomination === d.value
                          ...
                          : "border-[#ff6b00]/30 hover:border-[#ff6b00]/40"
                      }`}
                    >
                      ...
                      <div className="flex gap-1">
                        <span className="text-[9px] bg-white/5 border border-[#ff6b00]/40 text-white/50 px-1.5 py-1 rounded-lg font-black">
                          🇹🇷 {d.TR.free} св
                        </span>
                        <span className="text-[9px] bg-white/5 border border-[#ff6b00]/40 text-white/50 px-1.5 py-1 rounded-lg font-black">
                          🇮🇳 {d.IN.free} св
            ...
            <div className="bg-[#0a1860]/60 border border-[#ff6b00]/40 rounded-[2rem] p-6 flex-1">
              <div className="flex items-center justify-between mb-4">
                <p className="text-white/30 text-[10px] uppercase font-black tracking-widest">
                  Все ключи
                </p>
                <div className="flex gap-2">
                  <span className="text-[10px] bg-[#ff6b00]/10 border border-[#ff6b00]/40 text-[#ff6b00] px-2 py-1 rounded-lg font-black">
                    {freeCount} своб
                  </span>
                  <span className="text-[10px] bg-white/5 border border-[#ff6b00]/40 text-white/30 px-2 py-1 rounded-lg font-black">
                    {usedCount} исп
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2 max-h-[400px] overflow-y-auto pr-1">
                {filteredVouchers.map((v) => (
                  <div
                    key={v.id}
                    className={`flex items-center justify-between gap-2 p-3 rounded-xl border ${v.is_used ? "border-[#ff6b00]/30 bg-white/[0.01] opacity-40" : "border-[#ff6b00]/10 bg-[#ff6b00]/[0.02]"}`}
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      <Key
                        size={12}
                        className={
                          v.is_used ? "text-white/20" : "text-[#ff6b00]"
                        }
                      />
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      {v.denomination > 0 && (
                        <span className="text-[9px] bg-[#ff6b00]/10 border border-[#ff6b00]/40 text-[#ff6b00] px-1.5 py-0.5 rounded-lg font-black">
