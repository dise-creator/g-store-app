        ...
        <div className="flex items-center gap-4 mb-10">
          ...
          <div>
            <h1 className="text-3xl font-black uppercase text-white tracking-tighter">
              Импорт <span className="text-[#00d68f]">игр</span>
            </h1>
            <p className="text-white/30 text-xs mt-1">
              Поиск через RAWG API — одна кнопка добавляет игру в базу
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-3 mb-8">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-11 pr-5 py-4 bg-[#0a1860]/60 border border-[#00d68f]/40 rounded-2xl text-white placeholder-white/20 font-bold text-sm focus:outline-none focus:border-[#00d68f]/40 transition-all"
            />
          </div>
          <button
            onClick={search}
            disabled={loading}
            className="px-8 py-4 bg-[#00d68f] text-black font-black text-sm uppercase rounded-2xl hover:shadow-[0_0_20px_rgba(99,243,247,0.3)] transition-all active:scale-95 disabled:opacity-50 flex items-center gap-2"
          >
            {loading ? (
            ...
            >
              {results.map((game, i) => {
                ...
                return (
                  ...
                  >
                    <div className="relative aspect-[3/4] rounded-2xl overflow-hidden border border-[#00d68f]/30 bg-[#0a1860]/40">
                      ...
                      {isAdded && (
                        <div className="absolute inset-0 bg-[#00d68f]/20 flex items-center justify-center">
                          <Check size={32} className="text-[#00d68f]" />
                        </div>
                      )}
                    </div>

                    <p className="text-white/80 text-[10px] font-black uppercase truncate">
                      {game.name}
                    </p>
                    ...
                    <input
                      ...
                      placeholder="Цена ₽"
                      className="w-full px-3 py-2 bg-[#0a1860]/60 border border-[#00d68f]/40 rounded-xl text-white text-xs font-bold focus:outline-none focus:border-[#00d68f]/40 transition-all placeholder-white/20"
                    />
                    ...
                    <button
                      ...
                      className={`w-full py-2.5 rounded-xl font-black text-[10px] uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 ${
                        isAdded
                          ? "bg-[#00d68f]/10 border border-[#00d68f]/40 text-[#00d68f] cursor-default"
                          : isAdding
                            ? "bg-white/5 border border-[#00d68f]/40 text-white/30 cursor-wait"
