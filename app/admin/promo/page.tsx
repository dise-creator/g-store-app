        ...
        <div className="flex items-center gap-4 mb-10">
          ...
          <div>
            <h1 className="text-3xl font-black  uppercase text-white tracking-tighter">
              Промо<span className="text-[#ff6b00]">коды</span>
            </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-[#0a1860]/60 border border-[#ff6b00]/40 rounded-[2rem] p-6 flex flex-col gap-4">
            ...
            <div className="flex gap-2">
              {(["percent", "fixed"] as const).map((t) => (
                <button
                  key={t}
                  onClick={() => setType(t)}
                  className={`flex-1 py-3 rounded-2xl font-black text-xs uppercase  transition-all ${type === t ? "bg-[#ff6b00] text-black" : "bg-white/5 border border-[#ff6b00]/40 text-white/40 hover:text-white"}`}
                >
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-white/5 border border-[#ff6b00]/40 rounded-2xl text-white font-bold text-sm focus:outline-none focus:border-[#ff6b00]/40 transition-all placeholder-white/20"
            />
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-white/5 border border-[#ff6b00]/40 rounded-2xl text-white font-bold text-sm focus:outline-none focus:border-[#ff6b00]/40 transition-all"
            />
            ...
            <button
              ...
              className="w-full py-4 bg-[#ff6b00] text-black font-black uppercase  text-sm rounded-2xl hover:shadow-[0_0_30px_rgba(99,243,247,0.3)] transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {creating ? (
              ...
              <AnimatePresence>
                {promos.map((p) => (
                  <motion.div
                    ...
                    exit={{ opacity: 0 }}
                    className={`p-3 rounded-2xl border transition-all ${p.is_active ? "border-[#ff6b00]/10 bg-[#ff6b00]/[0.02]" : "border-[#ff6b00]/30 bg-white/[0.01] opacity-40"}`}
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <button
                          ...
                          className="text-white/20 hover:text-[#ff6b00] transition-all"
                        >
                          {copied === p.code ? (
                            <Check size={12} />
                          ) : (
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      <span className="text-[#ff6b00] text-[10px] font-black">
