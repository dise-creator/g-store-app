...
function NewsPageCard({ item, index }: { item: NewsItem; index: number }) {
  ...
  return (
    <motion.article
      ...
      className="group flex flex-col md:flex-row bg-[#0a1860]/60 border border-[#00d68f]/30 rounded-[2.5rem] overflow-hidden hover:border-[#00d68f]/40 hover:bg-white/[0.05] transition-all"
    >
      ...
      <div className="flex flex-col flex-1 justify-between p-8 gap-6">
        ...
        {linkedGame && displayPrice !== null && (
          <div className="flex items-center justify-between gap-4 pt-6 border-t border-[#00d68f]/30">
            <div className="flex items-center gap-4">
              <div className="relative w-14 h-14 rounded-2xl overflow-hidden border border-[#00d68f]/40 shrink-0">
                <Image
    ...
    <main className="relative min-h-screen pt-40 pb-20 px-6 md:px-10">
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto">
        <div className="flex items-center gap-6 mb-12">
          <Link
            href="/"
            className="group flex items-center justify-center w-10 h-10 rounded-full border border-[#00d68f]/40 text-white/30 hover:text-[#00d68f] hover:border-[#00d68f]/40 hover:bg-[#00d68f]/5 transition-all shrink-0"
          >
            <ChevronLeft
              size={20}
          ...
          {loading
            ? Array(3)
                ...
                .map((_, i) => (
                  <div
                    key={i}
                    className="h-56 rounded-[2.5rem] bg-[#0a1860]/40 animate-pulse"
                  />
                ))
            : news.map((item, i) => (
