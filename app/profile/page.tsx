        ...
        <div className="lg:hidden mb-4">
          <div className="relative bg-white/[0.03] border border-white/10 rounded-[2rem] p-5 flex items-center gap-4 overflow-hidden">
            ...
            <div className="relative shrink-0">
              {session.user?.image ? (
                <img src={session.user.image} className="w-14 h-14 rounded-2xl border-2 z-10 relative" style={{ borderColor: levelColor + "60" }} alt="avatar" />
              ) : (
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl font-black border-2 z-10 relative" style={{ backgroundColor: levelColor + "20", borderColor: levelColor + "40", color: levelColor }}>
                  {session.user?.name?.[0]}
                </div>
              )}
              <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-[#63f3f7] rounded-full flex items-center justify-center z-20">
                <Shield size={10} className="text-black" />
              </div>
            </div>
            <div className="flex-1 min-w-0 z-10">
              <p className="text-white font-black italic truncate">{session.user?.name}</p>
          ...
          >
            ...
            <div className="relative bg-white/[0.03] border border-white/10 rounded-[2rem] p-6 flex flex-col items-center gap-4 overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                {session.user?.image ? (
                  <img src={session.user.image} className="relative w-20 h-20 rounded-full border-2 z-10" style={{ borderColor: levelColor + "60" }} alt="avatar" />
                ) : (
                  <div className="relative w-20 h-20 rounded-full flex items-center justify-center text-3xl font-black border-2 z-10" style={{ backgroundColor: levelColor + "20", borderColor: levelColor + "40", color: levelColor }}>
                    {session.user?.name?.[0]}
                  </div>
                )}
            ...
            <AnimatePresence mode="wait">
                ...
                >
                  ...
                  {loadingOrders ? (
                    <div className="flex items-center justify-center h-40">
                      <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} className="w-6 h-6 border-2 border-[#63f3f7] border-t-transparent rounded-full" />
                    </div>
                  ) : orders.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-48 gap-4">
                      ...
                      <Link href="/" className="px-6 py-3 bg-[#63f3f7] text-black text-xs font-black uppercase italic rounded-2xl">В магазин</Link>
                    </div>
                  ) : (
                          ...
                          <div className="flex flex-col gap-2">
                            {order.items.map((item, j) => (
                              <div key={j} className="flex justify-between items-center py-2 border-t border-white/5">
                                <span className="text-white/70 text-xs font-bold italic truncate max-w-[60%]">{item.title}</span>
                                <span className="text-[#63f3f7] text-xs font-black">{(item.price * item.quantity).toLocaleString()} ₽</span>
                              </div>
                            ))}
                          </div>
                          <div className="flex justify-between items-center pt-2 border-t border-white/10">
                            <span className="text-white/30 text-[10px] uppercase font-black tracking-widest">Итого</span>
                            <span className="text-white font-black text-lg">{Number(order.total_price).toLocaleString()} ₽</span>
                          </div>
                        </motion.div>
                      ))}
                    </div>
              ...
              {activeTab === "bonus" && (
                ...
                >
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="relative p-5 rounded-2xl overflow-hidden border" style={{ borderColor: levelColor + "30", background: levelColor + "08" }}>
                      <div className="flex items-center gap-4">
                        <motion.div animate={{ scale: [1, 1.1, 1] }} transition={{ duration: 2, repeat: Infinity }} className="text-4xl">{levelIcon}</motion.div>
                        <div>
                      ...
                      {[
                        ...
                        <div key={l.level} className={`flex items-center justify-between p-3 rounded-xl border transition-all ${loyalty.level === l.level ? "border-white/20 bg-white/[0.05]" : "border-white/5 bg-white/[0.01]"}`}>
                          <div className="flex items-center gap-2">
                            ...
                            {loyalty.level === l.level && <span className="text-[8px] bg-[#63f3f7]/10 border border-[#63f3f7]/20 text-[#63f3f7] px-1.5 py-0.5 rounded-lg font-black uppercase">текущий</span>}
                          </div>
                          <div className="flex items-center gap-3">
                            <span className="text-white/20 text-[10px] font-black hidden sm:block">от {l.threshold}</span>
