...
export default function SignInPage() {
  ...
  return (
    <main className="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-[#050505] py-10">
      ...
      <Link
        href="/"
        className="absolute top-6 right-6 p-3 bg-white/5 hover:bg-white/10 rounded-full border border-[#ff6b00]/40 transition-all z-50 group"
      >
        <X
          size={20}
      ...
      <div className="relative z-10 w-full max-w-[900px] mx-4 flex flex-col md:flex-row gap-6">
          ...
          <div className="flex flex-col gap-3">
            {BENEFITS.map((benefit, i) => (
              <motion.div
                ...
                className="flex items-center gap-3 p-3 bg-[#0a1860]/60 border border-[#ff6b00]/30 rounded-2xl"
              >
                <div className="w-8 h-8 rounded-xl bg-[#ff6b00]/10 border border-[#ff6b00]/40 flex items-center justify-center shrink-0">
                  <benefit.icon size={14} className="text-[#ff6b00]" />
                </div>
        ...
        <motion.div
          ...
          className="w-full md:w-[360px] px-8 py-8 bg-black/60 backdrop-blur-3xl border border-[#ff6b00]/40 rounded-[3rem] shadow-[0_0_100px_rgba(255,107,0,0.08)]"
        >
          {/* Табы */}
          <div className="flex gap-1 p-1 bg-[#0a1860]/60 border border-[#ff6b00]/30 rounded-2xl mb-6">
            {TABS.map((t) => (
              <button
                key={t.id}
                onClick={() => {
              ...
              >
                {tab === "register" && (
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-4 bg-white/[0.04] border border-[#ff6b00]/40 focus:border-[#ff6b00]/40 rounded-2xl text-white font-bold text-sm outline-none transition-all placeholder-white/20"
                    />
                  </div>
                )}
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Пароль"
                    className="w-full pl-10 pr-10 py-4 bg-white/[0.04] border border-[#ff6b00]/40 focus:border-[#ff6b00]/40 rounded-2xl text-white font-bold text-sm outline-none transition-all placeholder-white/20"
                  />
                  <button
                ...
                <motion.button
                  ...
                  disabled={loading}
                  whileTap={{ scale: 0.97 }}
                  className="w-full py-4 bg-[#ff6b00] text-black font-black uppercase text-sm rounded-2xl disabled:opacity-50 transition-all hover:shadow-[0_0_20px_rgba(99,243,247,0.3)] no-hover mt-1"
                >
                  {loading
                    ? "Подождите..."
                    : tab === "register"
                  ...
                  {tab === "login" ? (
                    <>
                      ...
                      <button
                        ...
                        className="text-[#ff6b00]/60 hover:text-[#ff6b00] transition-colors no-hover"
                      >
                        Зарегистрироваться
                      </button>
                    </>
          ...
          <p className="mt-6 text-center text-[8px] text-white/10 uppercase font-bold tracking-[0.2em]">
            Нажмите{" "}
            <span className="text-white/20 border border-[#ff6b00]/40 px-2 py-0.5 rounded mx-1">
              ESC
            </span>{" "}
            для отмены
          </p>
